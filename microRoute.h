...
class MicroRouteDetection {
 ...
 private:
  ...
  std::map<RouteIndex, uint8_t> bfTxRxSnrReport;
  ...
  std::map<RouteIndex, uint8_t> bfTxRxClusterIdxReport;

  // Use to store a list of cluster reports
  std::list<clusterReport> bfTxRxClusters;

  // this multimap is used for micro-route discovery
  std::multimap<uint8_t, RouteIndex> bfSnrReportMicroRoute;

  // this multimap is used to store final micro-route indices
  std::multimap<uint8_t, RouteIndex> microRoute;

  std::list<RouteIndex> gridDeltaValues;
